Declare @ValorTaxaT int, @Hoje date
	...
	set @Hoje = (select pdate from datum where mpehotel = 2)
	...
	set @ValorTaxaT = (select betrag from splittab where @Hoje between fromdate and todate and stabnr=22)
		...
		select
			(select
					...
					isnull((select sum(epreis*anzahl) from leist where buchnr=buch.buchnr and rkz=0),0) as 'balance',
					resstat.resbez as 'resStatus',
					case when exists (select * from rechlog where CustomerID=buch.kundennr and rechlog.CustomerTaxID=kunden.vatno and Fisccode in (select ref from fiscalcdsaft where exporta='S')) then 1 else 0 end as 'BlockedVatNO'
			...
			inner join resstat on resstat.resnr=buch.resstatus
			where buch.mpehotel = 2   and buchstatus = 0 and buch.resstatus not in (3,7) and datumvon = (select pdate from datum where mpehotel = 2  ) and kat.zimmer=1
		FOR JSON PATH